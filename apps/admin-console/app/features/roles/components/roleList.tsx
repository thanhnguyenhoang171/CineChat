  ...
  const { mutate: deleteMutate, isPending: isDeletePending } = useMutation({
    ...
    onSuccess: (response) => {
      if (response.status === 200) {
        toast.success(
          t('role:deleteDialog.success', 'Role deleted successfully'),
        );
  ...
  return (
    <div className='flex flex-col h-full w-full justify-center'>
                  ...
                  {isVisible('actions') && (
                    <TableCell className='px-4 py-3'>
                      <div className='flex flex-row gap-1 justify-center items-center'>
                        <AppModifyButton
                          handleOnClick={() => handleEditClick(role)}
                        />
                        <AppDeleteButton
                          handleOnClick={() => handleDeleteClick(role)}
                        />
                      </div>
                    </TableCell>
                  )}
      ...
      <AppAlertDialog
        ...
        description={t('role:deleteDialog.description', {
