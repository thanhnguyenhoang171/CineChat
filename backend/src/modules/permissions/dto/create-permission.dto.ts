...
export class CreatePermissionDto {
  ...
  @IsEnum(HttpMethod, {
    message: `Phương thức HTTP phải là một trong các giá trị sau: ${Object.values(HttpMethod).join(', ')}`,
  })
...
export class SignPermissionToUserDto {
  @ApiProperty({
    example: ['role1', 'role2'],
    description: 'Danh sách ID role cần gán quyền',
  })
  @IsArray({ message: 'roleIds phải là mảng' })
  @IsString({ each: true, message: 'Mỗi ID role phải là chuỗi' })
  @IsNotEmpty({ each: true, message: 'ID role không được để trống' })
  roleIds: string[];
}
