    ...
    <div className='lg:col-span-4 flex flex-col gap-6'>
      <div className='rounded-xl border bg-white p-6 shadow-sm'>
        <div className='flex flex-col items-center text-center'>
          <UserAvatar
            user={account}
            className='mb-4 size-32 border-4 border-slate-50 shadow-sm rounded-full overflow-hidden'
          />

          {account?.username && (
            <h2 className='text-xl text-slate-900'>{account?.username}</h2>
          )}
          {account?.email && (
          ...
          <div className='w-full border-t pt-4'>
            <p className='text-sm text-slate-600 leading-relaxed text-justify'>
              ...
              consectetur adipisicing elit. Quia eos nam omnis rerum nulla dolor
              ut sequi.
            </p>
          </div>
        </div>
