import { ArrowDown, ArrowUp, ArrowUpDown, TriangleAlert, Star, Calendar, Clock } from 'lucide-react';
...
export function MovieList({
  ...
  const getStatusBadgeVariant = (status: string) => {
    switch (status) {
      ...
      case 'Post Production': return 'bg-purple-100 text-purple-700 border-purple-200';
  ...
  return (
    <div className='flex flex-col h-full w-full justify-center'>
              ...
              {tableHeaders.map((header) => (
                <TableHead
                  ...
                  <div className={cn(
                    ...
                    ['actions', 'status', 'voteAverage'].includes(header.key) ? 'justify-center' : 'justify-start'
                  )}>
                    {t(header.label)}
                    {header.sortable && (
                      <span className='ml-1'>
          ...
          <TableBody>
            {isLoading ? (
              ...
              moviesList.map((movie) => (
                <TableRow key={movie._id} className='hover:bg-muted/30 transition-colors group'>
                  {/* Title & Poster */}
                  <TableCell className='px-4 py-3'>
                    <div className='flex items-center gap-3'>
                      <div className='relative shrink-0'>
                        <Avatar className='h-12 w-9 border border-border rounded-sm overflow-hidden'>
                          <AvatarImage src={movie.posterPath} alt={movie.title} className='object-cover' />
                  ...
                  <TableCell className='px-4 py-3 text-center'>
                    <div className='flex flex-col items-center justify-center gap-0.5'>
                      <div className='flex items-center gap-1'>
                        ...
                        <span className='text-sm font-bold text-slate-700'>{movie.voteAverage?.toFixed(1) || '0.0'}</span>
                      </div>
                      <span className='text-[10px] text-muted-foreground'>{movie.voteCount} votes</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className='px-4 py-3'>
                    <div className='flex items-center justify-center gap-1'>
                      <AppModifyButton handleOnClick={() => handleEditClick(movie)} />
                      <AppDeleteButton handleOnClick={() => handleDeleteClick(movie)} />
                    </div>
                  </TableCell>
                </TableRow>
              ))
      ...
      <AppAlertDialog
        ...
        description={t('movie:deleteDialog.description', { name: movieToDelete?.title })}
