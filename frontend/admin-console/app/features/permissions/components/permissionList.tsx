  ...
  const { mutate: deleteMutate, isPending: isDeletePending } = useMutation({
    ...
    onSuccess: (response) => {
      if (response.status === 200) {
        toast.success(t('permission:deleteDialog.success', 'Permission deleted successfully'));
  ...
  return (
    <div className='flex flex-col h-full w-full justify-center'>
                ...
                return (
                  <TableHead
                    ...
                    <div className={cn(
                      ...
                      header.key === 'actions' || header.key === 'isActive' || header.key === 'method' ? 'justify-center' : 'justify-start'
                    )}>
                      {t(header.label)}
                      {header.sortable && (
                        <span className='ml-1'>
          ...
          <TableBody>
            {isLoading ? (
              Array.from({ length: pageSize }).map((_, index) => (
                <TableRow key={index}>
                  {PermissionTableHeader.map((header) => isVisible(header.key) && (
              ...
              permissionsList.map((permission) => (
                <TableRow key={permission._id} className='hover:bg-muted/30 transition-colors'>
                  {/* ID */}
                  {isVisible('_id') && (
                    <TableCell
                      className={cn(
                  ...
                  {/* Name */}
                  {isVisible('name') && (
                    <TableCell className='px-4 py-3 font-bold text-sm text-foreground/80'>{permission.name || '--'}</TableCell>
                  )}
                  ...
                  {isVisible('module') && (
                    <TableCell className='px-4 py-3'>
                      <Badge variant='outline' className='text-[10px] font-bold border-primary/20 text-primary/80 bg-primary/5 capitalize'>
                        {permission.module || '--'}
                      </Badge>
                    </TableCell>
                  )}
                  ...
                  {isVisible('actions') && (
                    <TableCell className='px-4 py-3'>
                      <div className='flex flex-row gap-1 justify-center items-center'>
                        <AppModifyButton handleOnClick={() => handleEditClick(permission)} />
                        <AppDeleteButton handleOnClick={() => handleDeleteClick(permission)} />
                      </div>
                    </TableCell>
                  )}
                </TableRow>
      ...
      <AppAlertDialog
        ...
        description={t('permission:deleteDialog.description', { name: permissionToDelete?.name })}
        onConfirm={() => permissionToDelete && deleteMutate(permissionToDelete._id)}
