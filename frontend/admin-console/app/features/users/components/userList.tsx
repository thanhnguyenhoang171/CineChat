import { ArrowDown, ArrowUp, ArrowUpDown, TriangleAlert, Mail, Shield } from 'lucide-react';
  ...
  return (
    <div className='flex flex-col h-full w-full justify-center'>
              ...
              {tableHeaders.map((header) => (
                <TableHead
                  ...
                  <div className={cn(
                    ...
                    header.key === 'actions' || header.key === 'isActive' ? 'justify-center' : 'justify-start'
                  )}>
                    {t(header.label)}
                    {header.sortable && (
                      <span className='ml-1'>
              ...
              usersList.map((user) => (
                <TableRow key={user._id} className='hover:bg-muted/30 transition-colors'>
                  ...
                  <TableCell className='px-4 py-3'>
                    <div className='flex items-center gap-3'>
                      <Avatar className='h-9 w-9 border border-border'>
                        <AvatarImage src={user.picture?.url} alt={user.firstName} />
                  ...
                  <TableCell className='px-4 py-3 text-center'>
                    {user.isActive === 1 ? (
                      <Badge variant='outline' className='bg-green-500/10 text-green-600 border-green-200 text-[10px] font-bold uppercase py-0 px-2'>
                        {t('app:status.active')}
                      </Badge>
                    ) : (
                      <Badge variant='outline' className='bg-slate-500/10 text-slate-500 border-slate-200 text-[10px] font-bold uppercase py-0 px-2'>
                  ...
                  <TableCell className='px-4 py-3'>
                    <div className='flex items-center justify-center gap-1'>
                      <AppModifyButton handleOnClick={() => handleEditClick(user)} />
                      <AppDeleteButton handleOnClick={() => handleDeleteClick(user)} />
                    </div>
                  </TableCell>
                </TableRow>
              ))
      ...
      <AppAlertDialog
        ...
        description={t('user:deleteDialog.description', { name: `${userToDelete?.firstName} ${userToDelete?.lastName}` })}
